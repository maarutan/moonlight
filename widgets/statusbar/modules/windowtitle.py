...
class WindowTitleWidget(Box):
    def __init__(self, statusbar: "StatusBar"):
        ...
        config = self.confh.config_modules["windowtitle"]
        if not self.confh.is_vertical():
            config = merge(config, config["if-vertical"])

        self.conf_type: str = config["type"]
        self.conf_vertical_length: int = int(
            config.get("vertical-length", config["vertical-length"])
        )
        self.conf_title_length: int = int(config["title-length"])

        self.conf_unknown_title: str = str(config["unknown-title"]) or "Unknown"
        self.conf_icon = config["icon"] or {}
        self.conf_icon_enabled: bool = bool(self.conf_icon["enabled"])
        self.conf_icon_size: int = int(self.conf_icon["size"])
        self.conf_icon_position: str = str(self.conf_icon["position"]).lower()
    ...
    def create_title(
        ...
        if self.confh.is_vertical():
            stack_box = Box(
                name="statusbar-window-title-box-vertical",
                h_expand=True,
                v_expand=True,
                h_align="center",
                v_align="center",
            ...
            title_label = Label(label=text, name="statusbar-window-title-title")
        ...
        main_box = Box(
            name="statusbar-window-title-box",
            h_expand=True,
            v_expand=True,
            h_align="center",
            v_align="center",
