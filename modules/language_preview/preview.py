...
from .config import LanguagePreviewConfig

widget_name = "language-preview"
confh = LanguagePreviewConfig(widget_name)
...
if not enabled:
    ...
    class LanguagePreview(Window):
        ...
        def __init__(self):
            self.default_fullnames = confh.get_option(
                f"{widget_name}.default_fullnames", {}
            )
            self.replacer = {
                k.lower(): v
                for k, v in (confh.get_option(f"{widget_name}.replacer", {})).items()
            }
            self.hypr = Hyprland()
            ...
            super().__init__(
                ...
                margin=confh.get_option(f"{widget_name}.margin", "40px 40px 40px 40px"),
                layer=confh.get_option(f"{widget_name}.layer", "top"),
                anchor=confh.get_option(f"{widget_name}.position", "top right"),
                h_expand=True,
                v_expand=True,
                v_align="center",
                h_align="center",
        ...
        def _score_locale(self, locale_norm: str, active_norm: str) -> int:
            ...
            alias = confh.get_option(f"{widget_name}.languages", {})
        ...
        def on_layout_change(self, *args):
            ...
            if (
                ...
                self._call_id += 1
                self._schedule_hide(self.HIDE_DELAY_MS)
                return False
