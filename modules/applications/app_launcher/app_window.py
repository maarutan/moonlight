...
class ALAppWindow(Box):
    ...
    def on_changed(self, e):
        """Обновить список приложений при изменении текста."""
        text = e.get_text()
    ...
    def on_activate(self, e):
        """Нажатие Enter: запустить первое приложение и корректно закрыть лаунчер."""
        # попытаться запустить первый результат
        self.apps_box.launch_by_order(0)
        # вызвать корректное закрытие (анимация + hide/close)
        try:
            self.conf.tools.close()
        except Exception:
            # на всякий случай попытаться быстро скрыть
            try:
                self.conf.tools.force_close()
            except Exception:
                pass
