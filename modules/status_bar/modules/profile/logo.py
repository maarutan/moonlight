...
class Logo(EventBox):
    def __init__(self, logo: dict, on_click=None, popup_active: bool = False):
        ...
        self.image_collapse_path = image_cfg.get("collapse_image", None)

        image_path = image_cfg.get("path", PLACEHOLDER_IMAGE.as_posix())
        self.collapse_icon = logo.get("collapse_icon", "󱞥")
        self.collapse_type = logo.get("collapse_type", "text")
        ...
        if logo_type == "image":
            ...
            if not path.exists():
                if image_path.startswith(("http://", "https://")):
                    if URL_AVATAR.exists():
                        path = URL_AVATAR
                    elif self.get_preview.is_image_url(image_path):
            ...
            if self.collapse_type == "image":
                ...
                if collapse_path_raw.startswith(("http://", "https://")):
                    ...
                    if collapse_cache_path.exists():
                        collapse_path = collapse_cache_path
                    elif self.get_preview.is_image_url(collapse_path_raw):
    ...
    def _apply_collapse(self, hover: bool):
        ...
        if self.popup_active:
            widget_to_show = self.collapse_image
        else:
            widget_to_show = self.collapse_image if hover else self.normal_image
